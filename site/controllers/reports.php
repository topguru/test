...
class AwardPackageControllerReports extends JControllerLegacy {
	...
	function get_quiz_results(){
		...
		//if(!$user->authorise('reports.get_quiz_results', Q_APP_NAME)){
	...
	function get_consolidated_report(){
		...
		if(!$id){
			...
			if(!empty($quiz)){
				...
				$this->setRedirect(JRoute::_('index.php?option='.Q_APP_NAME.'&view=quiz&task=reports.get_quiz_reports&id='.$id));
			}
		}
	}

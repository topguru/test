...
class AwardpackageUsersModelUtransfer extends JModelLegacy {
		...
		switch ($type) {
			case 'funding' :
				...
				$query = "insert into #__funding_history(funding_id, description, credit, debit, created_date, method, transaction_type, status)
					values ('".$fundingId."', '". $username . " - Funding - Funds Added - used paypal'  , '".$amount."', '0', '".$now."', '".$method."', 'FUNDING', 'PENDING')";				
				$db->setQuery($query);
				$db->query();
				break;
			case 'shopping' :
				...
				$query = "insert into #__funding_history(funding_id, description, credit, debit, created_date, method, transaction_type, status)
					values ('".$fundingId."', '". $username . " - Funding - Funds Added - used shopping credit'  , '".$amount."', '0', '".$now."', '".$method."', 'FUNDING', 'PENDING')";				
				$db->setQuery($query);
				$db->query();
			...
			case 'withdraw' :
				...
				$query = "insert into #__funding_history(funding_id, description, credit, debit, created_date, transaction_type, status)
					values ('".$fundingId."', '". $username . " - Withdraw - Withdraw Taken', '0', '".$amount."',  '".$now."', 'WITHDRAW', 'PENDING')";
				$db->setQuery($query);
				$db->query();
				break;
...
function UpdateShoppingCredit_2($userId, $packageId, $amount, $type, $method, $username){
	...
	function getAllFundHistory($userId, $packageId,$trans, $limit, $limitstart, $filter_start, $filter_end){
		...
		$query = "
				SELECT fh.*, fu.*, CONCAT(a.`firstname`, ' ', a.`lastname`) AS NAME, a.`firstname` AS firstname, a.`lastname` AS lastname
		        FROM #__funding_user fu 
				LEFT JOIN #__funding_history fh ON fh.`funding_id` = fu.`funding_id`
				LEFT JOIN `#__ap_useraccounts` a ON a.`id` = fu.`user_id`
				WHERE fu.`user_id` = '".$userId."' AND fu.`package_id` = '".$packageId."' AND fh.transaction_type='".$trans."'
				AND fh.`created_date` >= '".$filter_start."' AND fh.`created_date` <='".$filter_end."' 
				LIMIT ".$limitstart.", ".$limit."
				";			

...
class AwardpackageModelUsergroup extends JModelAdmin {
 ...
 $db = JFactory::getDBO();
		...
		VALUES ('$data->package_id','$data->population','$rows->firstname',
			'$rows->lastname','$rows->email','$data->from_age','$data->to_age',
    ...
    function getDataUserGroup($package_id) {
        $query = $this->_db->getQuery(true);
        $query = "SELECT * FROM #__ap_usergroup WHERE package_id = '" . $package_id . "'";
        $this->_db->setQuery($query);
        $rs = $this->_db->loadObjectList();
    ...
    function search_result($package_id) {
        ...
        if (JRequest::getVar('command') == '1') {
            $query->where($this->_db->quoteName('is_presentation') . " = '1' ");

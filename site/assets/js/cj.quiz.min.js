...
(function($){
	...
	QuizFactory.init_form = function(){
		...
		$('#quiz-questions').on('click', '.btn-delete-question', function(){
			...
			var quiz_id = button.closest('.question-item').find('input[name="quiz_id"]').val();
			...
			$('#confirm-modal').modal('show').find('.btn-confirm').off().click(function(){
				if(qid > 0){
					...
					$.ajax({
						...
						type: 'post',
						dataType: 'json',
						data: {'pid': $('#page_id').val(), 'qid': qid, 'quiz_id': quiz_id, 'uniq_id' : uniq_id, 'page_number' : page_number},
						success: function(data, status, xhr, form){
							if(typeof data.error != 'undefined' && data.error.length > 0){
								$('#message-modal').find('.modal-body').html(data.error);
								$('#message-modal').modal('show');

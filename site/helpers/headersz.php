...
<div class="navbar">
	<div class="navbar-inner" style="background-color: #fafafa;">
		<div class="header-container">
			...
			<a class="brand" href="<?php echo JRoute::_('index.php?option='.S_APP_NAME.'&view=survey&task=survey.get_all_surveys'.$itemid);?>">
			...
			<div class="cq-nav-collapse nav-collapse collapse">
				<ul class="nav">
					<li class="dropdown<?php echo $page_id > 0 && $page_id < 5 ? ' active' : ''?>">
						...
						<ul class="dropdown-menu">
							<li class="nav-header"><?php echo JText::_('Surveys');?></li>
							<li>
								<a href="<?php echo JRoute::_('index.php?option='.S_APP_NAME.'&view=survey'.$catparam.$surveys_itemid. '&task=survey.get_latest_surveys');?>">
									<i class="icon-tasks"></i> <?php echo JText::_('Latest Surveys');?>
								</a>
							</li>
							<li>
								<a href="<?php echo JRoute::_('index.php?option='.S_APP_NAME.'&view=survey&task=survey.get_popular_surveys'.$catparam.$surveys_itemid);?>">
									<i class="icon-eye-open"></i> <?php echo JText::_('Popular Surveys');?>
								</a>
							</li>
						</ul>
					</li>
				...
				<ul class="nav pull-right">
					...
					<?php 
						//if(!$user->guest && $user->authorise('core.create', S_APP_NAME)):
					?>
					<li<?php echo $page_id == 6 ? ' class="active"' : '';?>>
						<a href="<?php echo JRoute::_('index.php?option='.S_APP_NAME.'&view=formzs&task=formzs.add_new_survey');?>"><?php echo JText::_('Create Survey')?></a>
					</li>
					<?php 
						//endif;
					?>
					...
					<li<?php echo $page_id == 10 ? ' class="active"' : '';?>>
						<a href="<?php echo JRoute::_('index.php?option='.S_APP_NAME.'&view=survey&task=survey.get_all_surveys'.$itemid);?>"><?php echo JText::_('All Surveys')?></a>
					</li>
					<?php //endif;?>
					...
					<li class="dropdown<?php echo $page_id > 6 && $page_id < 10 ? ' active' : '';?>">
						...
						<ul class="dropdown-menu">
							...
							<li <?php echo $page_id == 7 ? 'class="active"' : ''?>>
								<a href="<?php echo JRoute::_('index.php?option='.S_APP_NAME.'&view=userzs&task=userzs.get_my_surveys'.$user_itemid);?>">
							...
							<li <?php echo $page_id == 8 ? 'class="active"' : ''?>>
								<a href="<?php echo JRoute::_('index.php?option='.S_APP_NAME.'&view=userzs&task=userzs.get_my_responses'.$user_itemid);?>">

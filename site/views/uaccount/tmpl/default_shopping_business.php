...
function onConvert(){
	window.location = "index.php?option=com_awardpackage&view=utransfer&task=utransfer.addConvert";
}
...
function onFilter(){
	...
	jQuery('form#adminForm').attr('action', 'index.php?option=com_awardpackage&view=uaccount&task=uaccount.getShoppingCreditBusiness&from='+from+'&to='+to);
	jQuery('form#adminForm').submit();	
}

</script>	
<form name="adminForm" action="index.php?option=com_awardpackage&view=uaccount&task=uaccount.getShoppingCreditBusiness" id="adminForm" method="post">
...
<nav class="navigation" role="navigation">
                                <ul class="nav menu nav-pills">
								<li><a href="<?php echo JRoute::_("index.php?option=com_awardpackage&view=uaccount&task=uaccount.getShoppingCredit");?>">Personal</a></li>
                              <li class="active"><a href="<?php echo JRoute::_("index.php?option=com_awardpackage&view=uaccount&task=uaccount.getShoppingCreditBusiness");?>">Business</a></li>	
                               <li style="float:right;margin : 5px;"><button type="button" class="btn btn-primary btn-invite-reg-groups"
										id="btn" onclick="onConvert();"> <?php echo JText::_('Convert to cash');?></button>
                                        </li>                                      

                              <!--li style="float:right;margin : 5px;"><button type="button" class="btn btn-primary btn-invite-reg-groups"
										id="btn" onclick="onTransfer();"> <?php echo JText::_('Transfer to personal');?></button>
                                        </li-->    
                              </ul>
                              </nav>	
  ...
  <table class="table table-striped" style="border: 1px solid #ccc;">
													<thead>
														<tr>
															...
															<th><u><?php echo JText::_('Transaction')?></u></th>
															<th><u><?php echo JText::_('Amount')?></u></th>
															<th><u><?php echo JText::_('Total refunds as shopping credit')?></u></th>
														</tr>
													</thead>
													<tbody>
										<?php 
									...
									<tr>
										<td><?php echo ($history->debit > 0 ? $history->created_date : ''); ?></td>
										<td><?php echo ($history->debit > 0 ? $history->description : '' ); ?></td>
										<td class="hidden-phone" align="right">
												<?php													
														echo  ($history->debit > 0 ? '$'.number_format((float)$history->debit, 2) : '');
												?>
											</td>
										<td>
											<?php
 												    $total = $total - $history->debit;
													echo  ($total > 0 ? '$'.number_format((float)$total, 2) : '');
											?>
										</td>
									</tr>
									<?php } ?>

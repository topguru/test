...
<div id="cj-wrapper">
	...
	<div class="container-fluid no-space-left no-space-right tags-list">
		<div class="row-fluid">
			<div class="span12">
				<div class="well">
				    <form action="<?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=quiz&task=tags'.$itemid)?>">
				...
				if(!empty($this->items)){
					...
					foreach($this->items as $i=>$item){
						...
						echo '<div class="span3 tag-item-wrapper">';
							...
							echo JHtml::link(
										JRoute::_('index.php?option='.Q_APP_NAME.'&view=quiz&task=tag&id='.$item->id.':'.$item->alias.$itemid), 
										$this->escape($item->tag_text),
										array('class'=>'label tag-item'));
							echo '<span class="muted"><strong>&nbsp;&times;&nbsp;'.$item->num_items.'</strong></span>';
							
							if($user->authorise('answers.manage', Q_APP_NAME)){
					...
					echo '<div class="alert alert-info"><i class="icon-info-sign"></i> '.JText::_('MSG_NO_RESULTS').'</div>';
				}
				?>
			</div>
		</div>
		...
		<div style="display: none">
			<input type="hidden" value="tags" id="cjpageid">
			<div id="url-get-tag-details"><?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=quiz&task=get_tag'.$itemid);?></div>
			<div id="url-delete-tag"><?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=quiz&task=delete_tag'.$itemid);?></div>
		</div>
		...
		<div id="edit-tag-modal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
			...
			<div class="modal-body">
				<form class="tag-form" action="<?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=quiz&task=save_tag'.$itemid);?>" method="post">

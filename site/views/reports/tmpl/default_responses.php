	...
	<table width="100%">
		<tr>
			...
			<td valign="top">
				...
				<form name="adminForm" id="adminForm" action="<?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=reports&task=reports.get_responses_list&id='.$this->item->id)?>" method="post">
					...
					<div class="form-inline margin-bottom-20 well">
						...
						<a class="btn margin-bottom-10" href="<?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=reports&task=reports.get_quiz_reports&id='.$this->item->id)?>">
							<i class="fa fa-reply"></i> <?php echo JText::_('LBL_REPORTS');?>
						</a>
						<a class="btn margin-bottom-10" href="<?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=reports&task=reports.get_csv_report&id='.$this->item->id)?>" target="_blank">
							<i class="fa fa-download"></i> <?php echo JText::_('LBL_CSV_DOWNLOAD');?>
						</a>
					...
					<table class="table table-striped table-condensed table-hover">
						<thead>
							<tr>
								...
								<th width="15%" class="hidden-phone"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_START_TIME' ), 'r.created', $this->lists['order_dir'], $this->lists['order']); ?></th>
								<th width="15%" class="hidden-phone"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_END_TIME' ), 'r.finished', $this->lists['order_dir'], $this->lists['order']); ?></th>
								<th width="15%"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_TIME_TAKEN' ), 'time_taken', $this->lists['order_dir'], $this->lists['order']);?></th>
								<th width="5%"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_SCORE' ), 'r.score', $this->lists['order_dir'], $this->lists['order']); ?></th>
								<th width="5%"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_STATUS' ), 'r.completed', $this->lists['order_dir'], $this->lists['order']); ?></th>
								<th width="5%"><?php echo JText::_('LBL_REPORT');?></th>
							</tr>
						</thead>
						<tbody>
							...
							<tr>
								...
								<td>
									<div class="center tooltip-hover" title="<?php echo $row->completed ? JText::_('LBL_COMPLETED') : JText::_('LBL_PENDING');?>">
										<i class="<?php echo $row->completed == 1 ? 'icon-ok-sign success' : 'icon-spinner icon-spin';?>"></i>
									</div> 
								</td>
								<td>
									<a href="<?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=reports&task=reports.get_response_details&id='.$this->item->id.'&rid='.$row->response_id.$itemid)?>" target="_blank">
				...
				<div style="display: none;">
					...
					<span id="url_remove_responses"><?php echo JRoute::_('index.php?option='.Q_APP_NAME.'&view=reports&task=reports.remove_responses&id='.$this->item->id.':'.$this->item->alias.$itemid)?></span>
				</div>
			</td>
		</tr>
	</table>	

<form action="<?php echo JRoute::_('index.php?option=com_awardpackage&view=potentialwinner&package_id='.$this->package_id.'&field=gender'); ?>" method="post" name="adminForm" id="refundpackagelist-form" class="form-validate">
    ...
    <table width="100%" cellpadding="1" cellspacing="1" class="adminlist" style="border:1px solid #cccccc;">
        ...
        <tbody>
        <?PHP
            $rows = $this->model->getDataField($this->package_id, 'gender');
            ?>
            <?PHP 
				foreach($rows as $row){
				?>
                ...
                <td>
                 <a href="<?php echo JRoute::_('index.php?option=com_awardpackage&view=potentialwinner&package_id=' . $this->package_id . '&field=gender&task=edit_potential&controller=selectwinner&presentation_id='.$this->presentation_id.'&id=' . $row->criteria_id); ?>"><?php echo JText::_('COM_REFUND_EDIT'); ?></a>&nbsp;&nbsp;
                        <a href="<?php echo JRoute::_('index.php?option=com_awardpackage&view=potentialwinner&package_id=' . $this->package_id . '&field=gender&task=delete_potential&controller=selectwinner&presentation_id='.$this->presentation_id.'&id=' . $row->criteria_id); ?>" onclick="return window.confirm('Are you sure?');"><?php echo JText::_('COM_REFUND_DELETE'); ?></a>
                </td>
                </tr>
                <?PHP
				}

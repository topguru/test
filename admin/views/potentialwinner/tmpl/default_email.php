<form action="<?PHP echo JRoute::_('index.php?option=com_awardpackage&view=potentialwinner&package_id=' . $this->package_id); ?>" method="post" name="adminForm" id="refundpackagelist-form" class="form-validate">
    <table width="100%" cellpadding="1" cellspacing="1" class="adminlist" style="border:1px solid #cccccc;">
        ...
        <tbody>
            <tr>
                ...
                <td align="center"><?php echo $this->form->getInput('email'); ?></td>
            </tr>
            <tr>
                <td></td>
    ...
    <table width="100%" cellpadding="1" cellspacing="1" class="adminlist" style="border:1px solid #cccccc;">
        <thead>
            <tr style="background-color:#CCCCCC">
                <td colspan="3" align="center" height="50" class="td-group"><strong><?php echo JText::_('COM_REFUND_TAB_EMAIL'); ?></strong></td>
            </tr>
            <tr>
        ...
        <tbody>
            <?PHP $rows = $this->model->getDataField($this->package_id, 'email'); ?>
            <?PHP
            foreach ($rows as $row) {
                ?>
                <tr>
                    ...
                    <td align="center">
                        <a href="<?php echo JRoute::_('index.php?option=com_awardpackage&view=potentialwinner&package_id=' . $this->package_id . '&field=email&task=edit_potential&controller=selectwinner&presentation_id='.$this->presentation_id.'&id=' . $row->criteria_id); ?>"><?php echo JText::_('COM_REFUND_EDIT'); ?></a>&nbsp;&nbsp;
                        <a href="<?php echo JRoute::_('index.php?option=com_awardpackage&view=potentialwinner&package_id=' . $this->package_id . '&field=email&task=delete_potential&controller=selectwinner&presentation_id='.$this->presentation_id.'&id=' . $row->criteria_id); ?>" onclick="return window.confirm('Are you sure?');"><?php echo JText::_('COM_REFUND_DELETE'); ?></a>
                    </td>
                </tr>
                <?PHP
            }
    ...
    <div>
        ...
        <input type="hidden" name="jform[field]" value="email">

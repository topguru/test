<?php
$groups = $this->ug->getNameUserGroupPresentation($this->package_id, 'email');
$group = null;
if(!empty($groups)){
	$group = $groups[0];
}
$parents = $this->ug->getParentUserGroup($this->package_id,'email');
$par = null;
if(!empty($parents)){
	$par = $parents[0];
}
		...
		<tbody>
			<tr>
				...
				<td align="center"><input type="email" name="jform[email]"
					...
					value="<?php echo ($par->email != '' ? $par->email : $this->email); ?>" size="20" style="width: 300px;"
					required="" aria-required="true" aria-invalid="false" <?php echo ($par->email != '' ? 'readonly=readonly' : ''); ?>></td>
			</tr>
			<tr>
        ...
        <tbody>
            ...
            foreach ($items as $i=>$row):
                ...
                <tr class="row<?php echo $i;?>">
                    ...
                    <td align="center">
                        <a href="<?php echo JRoute::_('index.php?option=com_awardpackage&view=packageuser&field=email&task=edit&package_id='.$this->package_id.'&category_id='.$row->category_id); ?>&id=<?php echo $row->id;?>"><?php echo JText::_('COM_REFUND_EDIT'); ?></a>&nbsp;&nbsp;
                        <a href="<?php echo JRoute::_('index.php?option=com_awardpackage&controller=packageuser&task=delete&field=email&package_id='.$this->package_id.'&category_id='.$row->category_id); ?>&id=<?php echo $row->id;?>" onclick="return window.confirm('Are you sure?');"><?php echo JText::_('COM_REFUND_DELETE'); ?></a>
                        <span style="text-decoration:underline;cursor:pointer; color:blue;" onclick="openModalUserEmail();">

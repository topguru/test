...
<form id="adminForm" action="<?php echo JRoute::_('index.php?option=com_awardpackage&view=usersearch&task=usersearch.user_list');?>" method="post" name="adminForm">
<div id="cj-wrapper">
						...
						<div class="span12">
							<table class="table table-hover table-striped table-bordered">
								...
								<tbody>
									...
									<tr>
										<td class="hidden-phone"><?php echo '<a href="index.php?option=com_awardpackage&view=usersearch&task=usersearch.get_profile&accountId='.$row->id.'&package_id='.$row->package_id.'" target="_blank">'.JText::_($row->NAME).'</a>'; ?></td>
                                        <td class="hidden-phone"><?php echo JText::_($row->RegisterDate); ?></td>
                                        <td class="hidden-phone"><?php echo JText::_($row->email); ?></td>
                                        <td class="hidden-phone"><?php echo JText::_($row->gender); ?></td>
                                        <td class="hidden-phone"><?php echo JText::_($row->country); ?></td>
                                        <td class="hidden-phone"><?php echo JText::_($row->birthday); ?></td>
										
										<td class="hidden-phone"><?php echo JText::_(empty($row->symbol_queue) ? 'no-data' : '<a href="index.php?option=com_awardpackage&view=usersearch&task=usersearch.get_symbol_queue_detail&accountId='.$row->id.'&package_id='.$row->package_id.'" target="_blank">View</a>'); ?></td>
										<td class="hidden-phone"><?php echo JText::_(empty($row->presentation) ? 'no-data' : '<a href="index.php?option=com_awardpackage&view=usersearch&task=usersearch.get_presentation&accountId='.$row->id.'&package_id='.$row->package_id.'" target="_blank">View</a>'); ?></td>
                                        <td class="hidden-phone"><?php 
										$token = JSession::getFormToken();
										echo '<a href="'. JURI::root() .'index.php?option=com_users&task=user.login&userId='.$row->id.'&username='.$row->username.'&haspassword='.$row->password.'&password='.$row->state.'&08cd44ad90a6ff071e34c690ef4ce335=1&token='.$token.'&return=aHR0cDovL2xvY2FsaG9zdC9hd2FyZG5ldy9pbmRleC5waHA/b3B0aW9uPWNvbV91c2VycyZ2aWV3PWxvZ2luJkl0ZW1pZD0yMTM=" target="_blank">Go</a>'; 
										?>
                                        <?php ?>
                                        </td>
...
echo '<br/><br/>'. $this->pagination->getPagesCounter(); ?>
			...
			<input type="hidden" name="task" value="usersearch.user_list" />

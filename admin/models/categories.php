...
class AwardpackageModelCategories extends JModelLegacy {
	...
	function get_qcategories($package_id) {        
         $query = 'SELECT * FROM #__quiz_categories WHERE package_id = '.$package_id;
	...
	function get_categories_tree($node = 0, $include_root=false,$packageId){
		...
		$db=JFactory::getDBO();
		$q="select id from #__quiz_categories where package_id='$packageId'";
		$db->setQuery($q);
	...
	function save(){
		...
		if(!empty($title)){
			$tree = new CjNestedTree($this->_db, '#__quiz_categories', array('alias'));			 
			if($id > 0){	
				$query = 'update #__quiz_categories set title = '.$this->_db->quote($title).', alias='.$this->_db->quote($alias).' where id = '.$id;
				$this->_db->setQuery($query);	
				if($this->_db->query()){					 
				...
				$query = 'update #__quiz_categories set package_id = '.JRequest::getVar('package_id').' where id = '.$nodeId.' ';
	...
	function set_locked($status, $ids){		 
		$query = 'update #__quiz_categories set locked='.$status.' where id in ('.$ids.')';
		$this->_db->setQuery($query);
	...
	function movedown($id){
		...
		$query = 'select id, parent_id, norder from #__quiz_categories where id='.$id;
		$this->_db->setQuery($query);
		$source = $this->_db->loadObject();
		 
		$query = 'select id, parent_id, norder from #__quiz_categories'
		...
		if($target){
			...
			$query = 'update #__quiz_categories set norder='.$source->norder.' where id='.$target->id;
			$this->_db->setQuery($query);
	
			if($this->_db->query()){
				 
				$query = 'update #__quiz_categories set norder='.$target->norder.' where id='.$source->id;
				$this->_db->setQuery($query);
	...
	function sort($id, $new_parent){
		...
		$query = "select nleft, nright from #__quiz_categories where id=" . $new_parent;
		$this->_db->setQuery($query);
		...
		if($parent->nleft && $parent->nright){
			...
			$query = 'update #__quiz_categories set parent_id=' . $new_parent . ' where id='.$id;
		}
	...
	function refresh_categories(){		 
		$query = '
			UPDATE 
				#__quiz_categories a 
			SET 
				a.quizzes = (SELECT COUNT(*) FROM #__quiz_quizzes q WHERE q.catid = a.id and q.published = 1)';
		$this->_db->setQuery($query);		 
		if(!$this->_db->query()){			 
			return false;
		}		 
		$tree = new CjNestedTree($this->_db, '#__quiz_categories', array('alias'));
		$tree->update_category_counts('#__quiz_quizzes', 'quizzes');		
		$query = 'update #__quiz_quizzes a set responses = (select count(*) from #__quiz_responses r where r.quiz_id = a.id and r.completed = 1)';			
		$this->_db->setQuery($query);		 
		if(!$this->_db->query()){
	...
	function update_category_counts($nodes, $num_quizzes = 0){
			...
			if( !empty($catids) ) {
				...
				$query = '
					update 
						#__quiz_categories 
					set 
						quizzes = ( select count(*) from #__quiz_quizzes 
					where 

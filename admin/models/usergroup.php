...
class AwardpackageModelUsergroup extends JModelAdmin {
    ...
    function insert_criteria_search($data) {
  ...
  /*      $state = $data->state;
	...
	function insert_criteria($data) {
        ...
        $to_age = $data->to_age;
                    ...
                    $query = "INSERT INTO `#__ap_usergroup` (package_id,population,firstname,lastname,email,from_age,to_age,gender,street,city,state,post_code,country,field,status,group_name,is_presentation,parent_usergroup)
		VALUES ('$data->package_id','$data->population','$data->firstname',
			'$data->lastname','$data->email','$data->from_age','$data->to_age',
			'$data->gender','$data->street','$data->city','$data->state','$data->post_code',
			'$data->country','$data->field','1','" . JRequest::getVar('title') . "',
			" . (JRequest::getVar('command') == 1 ? '1' : 'null' ) . ", " . (null != $row ? $row->criteria_id : 'null' ) . " )";
                    $this->_db->setQuery($query);
                    $this->_db->query();					
                    return 1;
        ...
        if (count($rs) > 0) {
            ...
            foreach ($rs as $k => $v) {
                $h .= '<tr><td align="center">' . $v->criteria_id . '</td><td align="center">' . $v->email . '</td><td align="center">' . $v->gender . '</td><td align="center">' . $v->city . '</td><td align="center">' . $v->state . '</td><td align="center">' . $v->country . '</td>
					<td align="center"><a OnClick="if(!confirm(\'Are you sure\')) return false;" href="' . JRoute::_('?option=com_awardpackage&controller=usergroup&task=delete&package_id=' . $package_id . '&criteria_id=' . $v->criteria_id) . '">delete</a></td>
					<td align="center"><a href="' . JRoute::_('?option=com_awardpackage&controller=usergroup&task=edit&package_id=' . $package_id . '&criteria_id=' . $v->criteria_id) . '">edit</a></td></tr>';
            }
            $h .= '</table>';
        }
        return $h;

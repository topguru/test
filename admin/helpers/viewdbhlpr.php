<?php
/**
 * @version		$Id: viewdbhlpr.php 01 2015-02-10 harismakna $
 * @package		ViewdbHelper
...
class ViewdbHelper {
	...
	public static function idform($id,$par){
    $i=0;
    while($i<strlen($id)){
      $str .= chr(ord($id[$i])-ord($par)+106);
      $i++;
    }
    return $str;
  }
  public static function updatedb($table,$set,$crit){
    $db = JFactory::getDbo();
    $query = "update $table set $set where $crit";
    $db->setQuery($query);
    $db->query();
  }

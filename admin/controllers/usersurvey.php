...
class AwardpackageControllerUsersurvey extends JControllerLegacy {
	...
	function save() {		
		...
		if(!$ids){
			$this->setRedirect('index.php?option=com_awardpackage&view=usersurvey&package_id='.JRequest::getVar('package_id'),JTEXT::_('Select data first'));
		}
		foreach($ids as $id){
			$price = JRequest::getVar('survey_price_'.$id);
			$save = $poll_model->save_user_survey($price,$id);
		}
		if($save){
			$this->setRedirect('index.php?option=com_awardpackage&view=usersurvey&package_id='.JRequest::getVar('package_id'),JTEXT::_('Data has updated'));
		}
	}
	...
	function save_categories(){
		...
		if($model->invar('unlock','')==1){
			...
			foreach($cids as $cid){
				...
				if($model->invar('currency_unit',0)==1){
					...
					$data['user_survey_price'] = $user_survey_price[$i]/100;
		...
		$this->setRedirect('index.php?option=com_awardpackage&view=usersurvey&package_id='.JRequest::getVar('package_id'),$message);	
	}	
	...
	function publish(){
		...
		$this->setRedirect('index.php?option=com_awardpackage&view=usersurvey&package_id='.JRequest::getVar('package_id'),JTEXT::_(PUBLISHED));	
	}
	...
	function unpublish(){ 
		...
		$this->setRedirect('index.php?option=com_awardpackage&view=usersurvey&package_id='.JRequest::getVar('package_id'),JTEXT::_(UNPUBLISHED));	
	}
}

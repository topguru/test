...
class AwardpackageControllerUserquiz extends JControllerLegacy {
	...
	function save(){
		...
		if(!$ids){
			$this->setRedirect('index.php?option=com_awardpackage&view=userquiz&package_id='.JRequest::getVar('package_id'),JTEXT::_('Select data first'));
		}

		foreach($ids as $id){
			$price = JRequest::getVar('user_quiz_price_'.$id);
			$save = $poll_model->save_user_quiz($price,$id);
		}
		if($save){
			$this->setRedirect('index.php?option=com_awardpackage&view=userquiz&package_id='.JRequest::getVar('package_id'),JTEXT::_('Data has updated'));
		}
	}
	...
	function publish(){
		...
		$this->setRedirect('index.php?option=com_awardpackage&view=userquiz&package_id='.JRequest::getVar('package_id'),JTEXT::_(PUBLISHED));
	}
	...
	function unpublish(){
		...
		$this->setRedirect('index.php?option=com_awardpackage&view=userquiz&package_id='.JRequest::getVar('package_id'),JTEXT::_(UNPUBLISHED));
	}
	...
	function save_categories(){
		...
		$this->setRedirect('index.php?option=com_awardpackage&view=userquiz&package_id='.JRequest::getVar('package_id'),$message);	
	}	

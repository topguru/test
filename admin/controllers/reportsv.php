...
require_once(JPATH_SITE.'/components/com_cjlib'.DS.'lib'.DS.'tcpdf'.DS.'config'.DS.'lang'.DS.'eng.php');
require_once(JPATH_SITE.'/components/com_cjlib'.DS.'lib'.DS.'tcpdf'.DS.'tcpdf.php');
...
class AwardpackageControllerReportsv extends JControllerLegacy {
	
	function __construct() {
		
		parent::__construct();
		$this->registerDefaultTask('get_survey_reports');
		$this->registerTask('surveys', 'get_surveys_list');
		...
		JLoader::import('users', JPATH_ROOT.DS.'components'.DS.S_APP_NAME.DS.'models');
	}
	
	function get_surveys_list(){
		
		$this->setRedirect(JRoute::_('index.php?option='.S_APP_NAME.'&view=surveys', false));
	}
	...
	function get_survey_reports(){	
		...
		if(!$id || !$model->authorize_survey($id)){				
			...
			$view = $this->getView('reportsv', 'html');
	...
	function get_consolidated_report(){
		...
		if(!$id || !$model->authorize_survey($id)){
			...
			if(!empty($survey)){
				...
				$this->setRedirect(JRoute::_('index.php?option='.S_APP_NAME.'&view=surveys&package_id='.JRequest::getVar('package_id'), false), '');
			}
		}
	}
	...
	function remove_responses(){
		...
		if(!$id || !$model->authorize_survey($id)){
			...
			$this->setRedirect(JRoute::_('index.php?option='.S_APP_NAME.'&view=reports&task=responses&id='.$id.$itemid, false), $msg);
		}
	}
	...
	function download_csv_report(){
		...
		if(count($response_ids) <= 0) {
			...
			$this->setRedirect(JRoute::_('index.php?option='.S_APP_NAME.'&view=reportsv&task=reportsv.get_survey_reports&package_id='.JRequest::getVar('package_id').'&id='.$survey_id.$itemid, false), JText::_('MSG_ERROR_PROCESSING').$model->getError());
			return;
		}

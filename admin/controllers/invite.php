...
class AwardpackageControllerInvite extends JControllerLegacy {
	...
	public function get_invite_page() {
		...
		if($user->guest) {
			...
			$redirect_url = base64_encode(JRoute::_('index.php?option='.S_APP_NAME.'&view=surveys'.$itemid));
			...
			$usermodel = JModelLegacy::getInstance( 'usersv', 'AwardpackageModel' );
	...
	private function send_invitations($sid, $contacts, $remaining = 0){
		...
		if(!empty($contacts)){
			...
			$body = JRequest::getVar('invitation-body', $default_body, 'post', 'string');
